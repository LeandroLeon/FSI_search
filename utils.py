...
class sortedFringe(Queue):
    """ Fringe that is sorted after append an element """
    def __init__(self):
        self.A = []
        self.start = 0
...
class FIFOQueue(Queue):
    ...
    def pop(self):
        ...
        if self.start > 5 and self.start > len(self.A) / 2:
            ...
            self.start = 0
        return e
    """ Mejora el rendimiento este pop porque tiene un buffer de 5 elementos
        para no estar moviendo la lista en todas las llamadas """
...
class subestFringe(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        list.sort(self.A, key=lambda item: item.path_cost + self.problem.h(item))
