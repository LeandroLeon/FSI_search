...
def graph_search(problem, fringe):
    ...
    while fringe:
        node = fringe.pop()
        if problem.goal_test(node.state):
            print "Nodos expandidos ->" + str(expanded)
            return node
...
def ramification_limitated_graph_search(problem):
    """ Search the ideal path (leeast cost) without information"""
    return graph_search(problem, sortedFringe())
